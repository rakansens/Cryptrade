...
import { useChartActions, useDrawingActions, useChartStoreBase } from '@/store/chart.store'
import { isDevelopment } from '@/config/env'
import type { DrawingData, DrawingPoint } from '@/types/drawing'
  ...
  useEffect(() => {
    ...
    const onDrawTrendline = (e: Event) => {
      ...
      if (detail?.multiple && Array.isArray(detail.drawings)) {
        ...
        detail.drawings.forEach((drawingData: DrawingData, index: number) => {
          if (drawingData?.points && Array.isArray(drawingData.points) && drawingData.points.length >= 2) {
            const normalizedPoints = drawingData.points.map((p: DrawingPoint | { time: number; price: number }) => ({
              time: Math.floor(p.time / 1000) as any,
              value: ('value' in p ? p.value : ('price' in p ? p.price : 0))
            })) as { time: number; value: number }[]
            
            const drawing: any = {
              id: drawingData.id || `trend-${Date.now()}-${index}`,
              type: drawingData.type || 'trendline',
              ...
              metadata: {
                ...
                ...(drawingData.metadata?.['description'] ? { description: String(drawingData.metadata['description']) } : {}),
                index,
                total: detail.drawings.length,
                autoGenerated: (drawingData.metadata?.['autoGenerated'] as boolean) ?? false
              }
            }
      ...
      if (detail?.points && Array.isArray(detail.points) && detail.points.length >= 2) {
        const normalizedPoints = detail.points.map((p: DrawingPoint | { time: number; price: number }) => {
          // Handle both DrawingPoint (with value) and legacy format (with price)
          const value = 'value' in p ? p.value : ('price' in p ? p.price : 0)
          if (typeof value !== 'number' || typeof p.time !== 'number') {
      ...
      if (detail?.points && Array.isArray(detail.points) && detail.points.length >= 2) {
        ...
        const normalizedPoints = detail.points.map((p: DrawingPoint | { time: number; price: number }) => {
          ...
          return {
            ...
            value: value // price → value に変換（lightweight-charts標準）
          }
        })
