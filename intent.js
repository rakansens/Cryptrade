...
exports.detectEmotionalTone = exports.determineAnalysisDepth = exports.extractSymbol = exports.detectSmallTalk = exports.detectMarketChat = exports.detectHelpRequest = exports.detectGreeting = exports.detectTradingAnalysis = exports.detectDrawingProposal = exports.detectProposalRequest = exports.detectPriceInquiry = exports.detectUIControl = exports.detectEntryProposal = exports.detectShortInput = exports.analyzeIntent = void 0;
/**
 * 統一された意図分析関数
...
function detectShortInput(userQuery, _queryLower) {
    // Handle empty input
    if (userQuery.trim() === '') {
        return {
            intent: 'conversational',
...
exports.detectShortInput = detectShortInput;
function detectEntryProposal(userQuery, queryLower) {
...
exports.detectEntryProposal = detectEntryProposal;
function detectUIControl(userQuery, queryLower) {
    ...
    if ((queryLower.includes('rsi') && queryLower.includes('見')) ||
        (queryLower.includes('サポート') && queryLower.includes('レジスタンス') && !queryLower.includes('引')) ||
        queryLower.includes('トレンド分析')) {
...
exports.detectUIControl = detectUIControl;
function detectPriceInquiry(userQuery, queryLower) {
    ...
    const specificPricePatterns = [
        ...
        /bitcoin\s*price/i,
        /\b(btc|eth|xrp|bnb|sol|ada)\s*(quote|prc|price)/i,
        /\bquote\s+(btc|eth|xrp|bnb|sol|ada)/i,
        /cuál.*precio.*bitcoin/i,
        /precio.*bitcoin/i,
        /比特币.*价格/,
        /价格.*多少/, // Chinese: "价格是多少"
    ];
...
exports.detectPriceInquiry = detectPriceInquiry;
function detectProposalRequest(userQuery, queryLower) {
...
exports.detectProposalRequest = detectProposalRequest;
function detectDrawingProposal(userQuery, queryLower) {
...
exports.detectDrawingProposal = detectDrawingProposal;
function detectTradingAnalysis(userQuery, queryLower) {
...
exports.detectTradingAnalysis = detectTradingAnalysis;
function detectGreeting(userQuery, queryLower) {
...
function detectHelpRequest(_userQuery, queryLower) {
...
exports.detectHelpRequest = detectHelpRequest;
function detectMarketChat(userQuery, queryLower) {
...
exports.detectMarketChat = detectMarketChat;
function detectSmallTalk(userQuery, queryLower) {
...
exports.detectSmallTalk = detectSmallTalk;
/**
 * シンボル抽出関数
 */
function extractSymbol(query) {
    ...
    // 既にUSDTが付いているかチェック
    const usdtMatch = cleanQuery.match(/\b([A-Z]{2,5})USDT\b/i);
    ...
    for (const symbol of symbols) {
        ...
        if (symbolRegex.test(cleanQuery)) {
            return symbol + 'USDT';
        }
    }
 ...
 * 分析深度決定関数
 */
function determineAnalysisDepth(query) {
...
exports.determineAnalysisDepth = determineAnalysisDepth;
/**
 * 感情トーン検出関数
 */
function detectEmotionalTone(query) {
    const queryLower = query.toLowerCase();
...
exports.detectEmotionalTone = detectEmotionalTone;
/**
 * ヘルプレスポンス生成
 */
