...
/**
 * Secure Semantic Embedding Service
 * 
 * テキストの意味的埋め込みベクトルを生成し、
 * セマンティック検索を可能にする
 * 
 * This version uses secure API key storage instead of plain environment variables
 */
...
export class SecureSemanticEmbeddingService extends BaseService {
  private static instance: SecureSemanticEmbeddingService;
  ...
  // Override the execute method to add OpenAI specific headers with secure API key
  ...
  static getInstance(): SecureSemanticEmbeddingService {
    if (!SecureSemanticEmbeddingService.instance) {
      SecureSemanticEmbeddingService.instance = new SecureSemanticEmbeddingService();
    }
    return SecureSemanticEmbeddingService.instance;
  }
  ...
  async generateEmbedding(text: string): Promise<EmbeddingResult> {
    ...
    if (cached) {
      logger.debug('[SecureEmbeddingService] Cache hit', { textLength: text.length });
      return {
        embedding: cached,
        model: this.model,
        tokensUsed: 0,
    ...
    try {
      ...
      logger.info('[SecureEmbeddingService] Embedding generated', {
      ...
      logger.error('[SecureEmbeddingService] Failed to generate embedding', {
        error: String(error),
        textLength: text.length,
      });
      throw error;
  ...
  async batchGenerateEmbeddings(texts: string[]): Promise<EmbeddingResult[]> {
    ...
    for (let i = 0; i < texts.length; i += batchSize) {
      ...
      try {
        ...
        logger.error('[SecureEmbeddingService] Batch generation failed', {
  ...
  clearCache(): void {
    ...
    logger.info('[SecureEmbeddingService] Cache cleared');
  }
...
export const secureEmbeddingService = SecureSemanticEmbeddingService.getInstance();