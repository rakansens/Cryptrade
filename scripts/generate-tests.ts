...
class TestGenerator {
  ...
  private exportedItems: ExportedItem[] = [];
  ...
  private detectFileType(): string {
    ...
    if (this.filePath.includes('/api/') || fileName.includes('.api.')) return 'api';
    if (fileName.includes('.store.') || fileName.includes('store')) return 'store';
    if (fileName.endsWith('.tsx') && !fileName.includes('.test.')) return 'component';
    
    return 'util';
  }

  private generateHookTest(): TestTemplate {
...
import { ${hookName} } from './${fileName}';`,
  ...
  private generateApiTest(): TestTemplate {
    const fileName = path.basename(this.filePath, path.extname(this.filePath));
...
import { rest } from 'msw';`;
  ...
  private generateMethodTest(method: string): string {
  ...
  private generateGenericApiTests(): string[] {
    return [
  ...
  private generateStoreTest(): TestTemplate {
    const fileName = path.basename(this.filePath, path.extname(this.filePath));
    const storeName = this.exportedItems.find(item => 
      item.name.includes('Store') || item.name.includes('store')
    )?.name || 'useStore';
...
import { ${storeName} } from './${fileName}';`,
  ...
  private generateComponentTest(): TestTemplate {
    const fileName = path.basename(this.filePath, path.extname(this.filePath));
    const componentName = this.exportedItems.find(item => 
      item.type === 'function' && item.name && item.name.length > 0 && item.name[0] === item.name[0]?.toUpperCase()
    )?.name || fileName;
...
import { ${componentName} } from './${fileName}';`,
  ...
  private generateUtilTest(): TestTemplate {
    const fileName = path.basename(this.filePath, path.extname(this.filePath));
  ...
  private formatTest(template: TestTemplate): string {
