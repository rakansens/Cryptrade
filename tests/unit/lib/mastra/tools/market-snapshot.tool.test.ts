...
ÈìóÉgY');
    ...
    it('should determine bearish market mood', async () => {
      ...
      expect(result.marketHighlight).toContain('4oNÍjÕM’‹[fD~Y');
    });
  ...
  describe('execute - error handling', () => {
    ...
    it('should handle dynamic import failure', async () => {
      ...
      expect(result).toEqual({
        ...
        topGainers: [],
        topLosers: [],
        marketHighlight: '4Çü¿nÖ—k B„jOLLzWfD~Y',
        totalMarketCap: 0,
        btcDominance: 0,
      });
    });
...
describe('trendingTopicsTool', () => {
  ...
  describe('execute', () => {
    it('should return mock trending topics', async () => {
      ...
      expect(result).toEqual({
        topics: [
          { topic: 'Bitcoin ETF…', sentiment: 'positive', volume: 8500 },
          { topic: '¤üµê¢à¢Ã×°ìüÉ', sentiment: 'positive', volume: 6200 },
          { topic: '67nøõ', sentiment: 'negative', volume: 4300 },
        ],
        summary: 'ÊåoBitcoin ETFnqLgaMŠgY4o…gÛŠ
LcfD~Y',
      });
    });
    ...
    it('should handle execution errors', async () => {
      ...
      })).rejects.toThrow('ÈìóÉÈÔÃ¯nÖ—k1WW~W_');
